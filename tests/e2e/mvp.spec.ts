...
test.describe('v0.5.120 phase-echarts + polish', () => {
  ...
  test('shell', async ({ page }) => {
    ...
    await expect(page.getByText(/v0\.5\.120/).first()).toBeVisible()
  ...
  test('signal echarts', async ({ page }) => {
    await bootCross(page)
    await openNav(page, '信号')
    await expect(page.locator('#signal-echarts, .echart-host').first()).toBeVisible({ timeout: 10000 })
