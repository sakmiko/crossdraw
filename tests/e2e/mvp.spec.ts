...
test.describe('v0.5.123 xsection-echarts + polish', () => {
  ...
  test('shell', async ({ page }) => {
    ...
    await expect(page.getByText(/v0\.5\.123/).first()).toBeVisible()
  ...
  test('xsection echarts', async ({ page }) => {
    await bootCross(page)
    await openNav(page, '断面')
    await expect(page.locator('#xsection-echarts, .echart-host').first()).toBeVisible({ timeout: 12000 })
