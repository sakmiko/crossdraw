...
test.describe('Crossdraw v0.5.28', () => {
  test('multi corridor compare chart', async ({ page }) => {
    ...
    await expect(page.getByText('多走廊带宽对比').first()).toBeVisible({ timeout: 10000 })
    await expect(page.getByRole('button', { name: '导出多走廊对比' })).toBeVisible()
